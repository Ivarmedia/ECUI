...
export class Layout {
    ...
    buildCss() {
        var structureCss = document.createElement('style');
        structureCss.setAttribute('type', 'text/css');
        structureCss.id = "ECUI-Structure-Style";
        ...
        if (structureCss.styleSheet) {
            structureCss.styleSheet.cssText = cssRules;
        } else {
            structureCss.appendChild(document.createTextNode(cssRules));
        }
    ...
    update(panels) {
        ...
        this.styleSheet.id = "ECUI-Layout-Style";
        ...
        if(areas.some(area => area == "header")) {
            cssRules = `
                .ECUI-Layout-Header-Area { width: 100%; height: 30px; position: relative; }
            `;
            navigationHeight = 30;
        }
        ...
        if(areas.some(area => area == "nav")) {
            cssRules += `
                .ECUI-Layout-Navigation-Bar-Area { width: 100%; height: 35px; position: relative; }
            `;
            navigationHeight += 35;
        }
